...
#' @param file_path directory path and outfile name (do not include .fasta)
#'
#' @return a fasta file containing cleaned sequences is written
#' @export
#'
#' @examples QC(Seqs.fasta, 125, /myDirectory/myFileName)
QC<- function(fasta_file, seq_length, file_path){
  library(seqinr)
  ...
  write.fasta(as.list(QC3file), names=myNames, file= paste0(file_path, ".fasta")) ##Writes a new fasta file containing the seqs with correct length of NA
}
